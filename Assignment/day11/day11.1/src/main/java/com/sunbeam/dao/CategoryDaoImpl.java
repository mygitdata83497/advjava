...
//		String jpql = "select c from Category c where c.categoryName=:nm";
		...
		//join fetch = query will be applied only on category
		//innerjoin= if there are no details of post in category it will not give category details also hence we are using left join instead of inner join
		// 1. get session from SF (getCurrentSession)
		Session session = getFactory().getCurrentSession();
		// 2. Begin a Tx
		Transaction tx = session.beginTransaction();
